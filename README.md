...
## 📁 Projektstruktur – Modulare Architektur

Luma ist **hochgradig modularisiert** für klare Verantwortlichkeitsaufteilung und einfache Wartung.

```
luma/
├── 📄 server.js                    # Hauptentrypoint (HTTP + HTTPS Server mit Security)
├── 📄 package.json                 # Dependencies Management
├── .env                            # Umgebungsvariablen (PostgreSQL, Ports, etc.)
│
├── 🧬 algorithmus/                 # Ranking & KI-Engine (19 Module)
│   ├── [Details siehe: README.COMPLETE.md]
│
├── 📱 autocomplete/                # Suchvorschläge & Instant Answers
│   ├── index.js
│   ├── core/                       # Haupt-Logik
│   ├── renderers/                  # Frontend-Renderer für verschiedene Result-Typen
│   ├── sources/                    # Externe Datenquellen (Wiki, DuckDuckGo, etc.)
│   ├── styles/                     # CSS/Styling
│   ├── utils/                      # Helferfunktionen
│   └── server/
│       └── routes/                 # API-Endpoints für Autocomplete
│
├── 📊 modules/                     # Feature-Module & Utilities (14 Sub-Module)
│   ├── event-tracker.js            # Search/Security/Performance Event Tracking
│   ├── semantic-content-moderation.js # KI-gestützte Inhaltsmoderation
│   │
│   ├── acronyms/                   # Abkürzungen-Engine (LOL → Laughing Out Loud)
│   ├── calculator/                 # Kalkulator-Widget
│   ├── community-moderation/       # Regelbasierter Spam- & Beleidigungsfilter
│   ├── currency_converter/         # Echtzeit-Währungsumrechnung
│   ├── emoji/                      # Emoji-Datenbank & Lookup
│   ├── feiertag-countdown/         # Deutsche Feiertags-Countdown
│   ├── fuzzy/                      # Fuzzy String Matching
│   ├── security-check/             # Domain-Security (Malware/Phishing Check)
│   ├── synonyms/                   # Multilinguales Synonym-Matching + Spell-Check
│   ├── unix-timestamp/             # Zeitstempel-Konvertierung
│   ├── watt_waechter/              # Stromverbrauch & Energiepreis-Tracking
│   ├── wetter/                     # Wetter-Integration (mehrere Quellen)
│   └── zahl-zu-wort/               # Zahlen → deutsche Wortkonvertierung
│
├── 🎨 navigations_tabs/            # Frontend Tab-Renderer (7 Module)
│   ├── alles.js                    # Haupt-Suchergebnisse (mit 👍/😐/👎 Votes)
│   ├── bilder.js                   # Bildergrid mit Lightbox
│   ├── videos.js                   # Video-Grid Renderer
│   ├── fragen.js                   # FAQ/Accordion-Stil Fragen
│   ├── knowledge-panel.js          # Wikipedia/Wikidata Info-Panels
│   ├── related-questions.js        # Ähnliche Fragen Box
│   └── community-lists.js          # Inline Community-Listen Anzeige
│
├── 🔒 config/                      # Sicherheit & System (3 Module)
│   ├── security-config.js          # Zentrale Security-Config (HTTPS, Ports, etc.)
│   ├── rate-limiter.js             # DDoS/Abuse Protection (express-rate-limit)
│   ├── security-middleware.js      # XSS, CSRF, SQL-Injection Protection
│   └── certs/                      # SSL/TLS Zertifikate
│
├── 📦 data/                        # Backend Data Management (5 Module)
│   ├── auth-manager.js             # Nutzer-Authentifizierung (bcryptjs)
│   ├── domain-authority.js         # Domain Authority Calculator (Memory-Cache 1h)
│   ├── paywall-manager.js          # Paywall Detection & Reports
│   ├── ratings-manager.js          # Nutzerbewertungen (lokal JSON)
│   └── votes-manager.js            # Anonyme 👍/😐/👎 Abstimmungen
│
├── 🖥️  server/                     # API Routes & Server-Logik
│   ├── routes/
│   │   ├── index.js                # Route-Registrierung
│   │   ├── search.js               # 🔍 /api/search (Haupt-Such-Endpoint)
│   │   ├── admin.js                # /api/admin/* (Analytics, Health, Security-Logs)
│   │   ├── admin-moderation.js     # /api/admin/moderation/* (Moderator-Queue)
│   │   ├── feedback.js             # /api/reviews, /api/votes
│   │   ├── paywall.js              # /api/paywall (Paywall-Reports)
│   │   ├── community.js            # /api/community/* (Lists, Reports, Blacklist)
│   │   ├── auth.js                 # /api/auth/* (Login, Register, Logout)
│   │   ├── user.js                 # /api/user/* (Profil, Blacklist, Stats)
│   │   └── tracking.js             # /api/klick, /api/verweilzeit (Pogo-Tracking)
│   │
│   ├── middleware/
│   │   └── requireAuth.js          # Auth-Check Middleware
│   │
│   └── helpers/
│       └── db-helpers.js           # DB-Utility-Funktionen
│
├── 🗄️  luma-cleaner/               # URL-Parameter Cleaning (npm local package)
│   ├── cleaner-logic.js            # Tracking-Parameter Removal
│   └── package.json
│
├── 🔄 cron/                        # Scheduled Tasks & Background Jobs
│   ├── index.js                    # Cron-Job Zentrale
│   ├── ip-cleanup.js               # Alte IP-Adressen & Sessions löschen
│   └── legacy-cron.js              # Legacy Cron-Jobs
│
├── 📚 migrations/                  # Database Schema Initialization
│   ├── check-ip-system.js          # IP-System Validierung
│   ├── create-admin-moderation-tables.js # Admin-Moderation Schema
│   ├── create-ai-moderation-system.js    # KI-Moderation Tabellen
│   ├── create-blacklist-system.js        # Blacklist-System Setup
│   ├── create-semantic-blacklist.js      # Semantische Blacklist Setup
│   ├── add-insult-patterns.js            # Beleidigungsmuster einfügen
│   └── ...weitere Setup-Skripte
│
├── 🧪 Fakten-Check/                # Fake-News & Fact-Checking
│   ├── processor.js                # Fakten-Verarbeitung
│   ├── check-schema.js             # Schema-Validierung
│   ├── search-fakten.js            # Fakten-Suche API
│   └── init-tables.js              # Tabellen-Initialisierung
│
...
│   └── ...weitere HTML/CSS/JS Assets
│
├── ⚙️  setting/                    # (Settings & Konfiguration)
│
├── 📄 LUMA_PROJEKT_ANALYSE.txt     # Projekt-Roadmap & Aufgaben
└── 📄 ALGORITHMUS_ERKLAERUNG.txt   # Detaillierte Algorithmus-Dokumentation
```

---

## 🔗 Wie die Architektur zusammenhängt

Lumas Backend ist modular aufgebaut, was Wartung und Skalierung vereinfacht:

- **Frontend** (`public/`): Pure HTML/CSS/JavaScript — keine Frameworks für maximale Performance
- **Search Engine Core** (`server/routes/search.js`): PostgreSQL Full-Text + Multi-Layer Ranking
- **Intelligence Layer** (`algorithmus/`): 19 spezialisierte Module für Ranking, Trends, Spam-Filter, Trust, etc.
- **Data Management** (`data/`): Auth, Votes, Ratings, URL-Blacklists
- **Web Crawler** (`crawler_new/`): Nutzt Playwright + Ollama für KI-Kategorisierung (≈250-1500 Seiten/Stunde)
- **Database** (PostgreSQL): Full-Text-Indexing, Community-Votes, Embeddings für semantische Suche
